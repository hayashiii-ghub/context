...
struct FileActionService {
    ...
    func chooseZipDestination() -> URL? {
        ...
        panel.nameFieldStringValue = "context-\(Date().compactTimestamp()).zip"
    ...
    func createZip(from items: [ShelfItem], destination: URL) throws {
        let stagingRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("context-\(UUID().uuidString)", isDirectory: true)
        let stagingItems = stagingRoot.appendingPathComponent("context Items", isDirectory: true)
        ...
        let temporaryDestination = destination.deletingLastPathComponent()
            .appendingPathComponent(".context-\(UUID().uuidString).zip")
        defer {
